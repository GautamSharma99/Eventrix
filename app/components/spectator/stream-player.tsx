...
export function StreamPlayer() {
  ...
  return (
    <div className="relative w-full h-full bg-secondary/30 overflow-hidden">
      ...
      {/* Top-left: Phase + agent dots overlay */}
      {phase !== "waiting" && (
        <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
          ...
          <div className="flex items-center gap-1 rounded-md bg-background/60 backdrop-blur-sm border border-border/50 px-2 py-1.5">
            {agents.map((agent) => (
              ...
              >
                <div
                  className={`size-5 rounded-full border-2 transition-all flex items-center justify-center text-[7px] font-bold font-mono ${
                    agent.alive ? "" : "opacity-30"
                  }`}
                  style={{
      ...
      {/* Right: Compact event feed overlay */}
      {feed.length > 0 && (
        <div className="absolute top-3 right-3 bottom-3 w-[260px] z-10 flex flex-col rounded-lg bg-background/70 backdrop-blur-sm border border-border/50 overflow-hidden">
                ...
                return (
                  <div
                    ...
                    className={`flex items-start gap-2 rounded px-2 py-1.5 ${
                      i === 0 ? "animate-in slide-in-from-top-2 fade-in duration-300 bg-card/80" : "bg-transparent"
                    }`}
                  >
